...
import network.libertychat.app.rns.api.model.DeliveryStatus
...
class RnsBackendIpcRoundTripTest {
    ...
    fun `setConnecting round-trips and the callState observer sees Connecting`() = runTest {
        ...
        client.telephony.callState.test {
            ...
            assertEquals(network.libertychat.app.rns.api.model.CallState.Idle, awaitItem())
            ...
            assertTrue(
                ...
                next is network.libertychat.app.rns.api.model.CallState.Connecting,
            )
            assertEquals(hash, (next as network.libertychat.app.rns.api.model.CallState.Connecting).identityHash)
            cancelAndIgnoreRemainingEvents()
        }
...
private class FakeRnsTelephony : RnsTelephony {
    ...
    private val _callState =
        kotlinx.coroutines.flow.MutableStateFlow<network.libertychat.app.rns.api.model.CallState>(
            network.libertychat.app.rns.api.model.CallState.Idle,
        )
    ...
    override val callState: kotlinx.coroutines.flow.StateFlow<network.libertychat.app.rns.api.model.CallState>
        get() = _callState
    ...
    fun emitCallState(state: network.libertychat.app.rns.api.model.CallState) {
    ...
    override suspend fun setConnecting(destinationHash: String) {
        ...
        _callState.value = network.libertychat.app.rns.api.model.CallState.Connecting(destinationHash)
    }
    override suspend fun setEnded() {
        _callState.value = network.libertychat.app.rns.api.model.CallState.Ended
    }
...
private class FakeRnsTelemetry : RnsTelemetry {
    private val emissions = MutableSharedFlow<network.libertychat.app.rns.api.model.LocationTelemetry>(extraBufferCapacity = 4)
    override suspend fun sendLocationTelemetry(
        destinationHash: ByteArray,
        telemetry: network.libertychat.app.rns.api.model.LocationTelemetry,
        sourceIdentity: Identity,
        iconAppearance: IconAppearance?,
    ): Result<MessageReceipt> = Result.failure(NotImplementedError())
    ...
    override val locationTelemetryFlow: SharedFlow<network.libertychat.app.rns.api.model.LocationTelemetry> =
...
private class FakeRnsTransportAdmin : RnsTransportAdmin {
    ...
    override fun setBatteryProfile(profile: network.libertychat.app.rns.api.model.BatteryProfile) {}
