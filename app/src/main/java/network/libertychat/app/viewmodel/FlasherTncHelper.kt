...
import network.libertychat.app.rns.host.flasher.FirmwareDownloader
...
internal class FlasherTncHelper(
    ...
    fun selectTncRegion(region: network.libertychat.app.data.model.FrequencyRegion) {
        state.update {
            it.copy(
    ...
    fun selectTncPreset(preset: network.libertychat.app.data.model.ModemPreset) {
        state.update {
            it.copy(
    ...
    fun handleFlashComplete(
        deviceInfo: network.libertychat.app.rns.host.flasher.RNodeDeviceInfo?,
            ...
            val defaultRegion =
                when (currentState.selectedBand) {
                    FrequencyBand.BAND_433 ->
                        network.libertychat.app.data.model.FrequencyRegions.regions
                            .find { it.id == "eu_433" }
                    else ->
                        network.libertychat.app.data.model.FrequencyRegions.regions
                            .find { it.id == "us_915" }
                }
            val defaultPreset = network.libertychat.app.data.model.ModemPreset.DEFAULT
            state.update {
                it.copy(
