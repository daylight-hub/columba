...
class PropagationNodeManagerTest {
    ...
    fun `triggerSync waits for relay state to load before checking relay`() =
        runTest {
            ...
            val mockSyncState =
                network.libertychat.app.rns.api.model.PropagationState(
                    state = 0,
                    stateName = "IDLE",
                    progress = 0.0f,
                    messagesReceived = 0,
    ...
    fun `setManualRelayByHash - disables auto-select`() =
        runTest {
            ...
            coEvery { contactRepository.addPendingContact(any(), any()) } returns
                Result.success(network.libertychat.app.data.repository.ContactRepository.AddPendingResult.AddedAsPending)
