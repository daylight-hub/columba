...
import network.libertychat.app.rns.api.util.StampGenerator
...
class PythonRnsRuntime(
    ...
    fun start(config: ReticulumConfig) {
        ...
        // effect.
        val probe = network.libertychat.app.rns.api.util.SharedInstanceProbe
    ...
    private fun featureUnsupportedDestination(hexHash: String): Nothing =
        throw network.libertychat.app.rns.api.RnsException(
            network.libertychat.app.rns.api.RnsError.IdentityNotFound(hexHash),
        )

    /** Throw [network.libertychat.app.rns.api.RnsError.BackendNotReady] if [start] hasn't run. */
    fun requireRunning() {
        if (!running.get()) {
            throw network.libertychat.app.rns.api.RnsException(
                network.libertychat.app.rns.api.RnsError.BackendNotReady,
            )
        }
    }
