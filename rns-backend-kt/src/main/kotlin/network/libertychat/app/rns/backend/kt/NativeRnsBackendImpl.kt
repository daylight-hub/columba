...
import network.libertychat.app.rns.api.model.DeliveryStatusEventStream
...
import network.libertychat.app.rns.api.model.LinkStatus
...
import network.libertychat.app.rns.api.util.AppDataParser
import network.libertychat.app.rns.api.util.Aspects
import network.libertychat.app.rns.api.util.LxmfFields
import network.libertychat.app.rns.api.util.ReactionWireCodec
import network.libertychat.app.rns.api.util.hexToBytes
import network.libertychat.app.rns.api.util.isUserVisibleChatMessage
import network.libertychat.app.rns.api.util.toHex
import network.libertychat.app.rns.backend.kt.BuildConfig
...
import network.libertychat.app.rns.api.model.Destination as ColumbaDestination
import network.libertychat.app.rns.api.model.Identity as ColumbaIdentity
import network.libertychat.app.rns.api.model.Link as ColumbaLink
...
class NativeRnsBackendImpl(
     ...
     */
    private val callLifecycleRecorder: network.libertychat.app.rns.api.call.CallLifecycleRecorder? = null,
) : network.libertychat.app.rns.api.RnsCore,
    network.libertychat.app.rns.api.RnsLxmf,
    network.libertychat.app.rns.api.RnsTelephony,
    network.libertychat.app.rns.api.RnsTelemetry,
    network.libertychat.app.rns.api.RnsNomadnet,
    network.libertychat.app.rns.api.RnsTransportAdmin {
    companion object {
    ...
    private fun warnIfOtherRnsInstance(config: ReticulumConfig) {
        if (network.libertychat.app.rns.api.util.SharedInstanceProbe
                .shouldShareInstance(config)
        ) {
            Log.w(
                TAG,
                "Another RNS instance detected on 127.0.0.1:" +
                    "${network.libertychat.app.rns.api.util.SharedInstanceProbe.DEFAULT_PORT}; " +
    ...
    override suspend fun sendLocationTelemetry(
        ...
        fields[LxmfFields.FIELD_TELEMETRY] =
            network.libertychat.app.rns.api.util.TelemeterCodec
                .packLocationTelemetry(telemetry)
        network.libertychat.app.rns.api.util.TelemeterCodec.packColumbaMeta(telemetry)?.let {
    ...
    override suspend fun reloadInterfaces(configs: List<network.libertychat.app.rns.api.model.InterfaceConfig>) {
        withContext(Dispatchers.IO) {
    ...
    override suspend fun reconnectRNodeInterface() {
        withContext(Dispatchers.IO) {
            ...
            if (config != null) {
                val rnodeConfigs = config.enabledInterfaces.filterIsInstance<network.libertychat.app.rns.api.model.InterfaceConfig.RNode>()
    ...
    override suspend fun requestNomadnetPage(
        ...
        formDataJson: String?,
        timeoutSeconds: Float,
    ): Result<network.libertychat.app.rns.api.model.NomadnetPageResult> = nomadNetHandler.requestNomadnetPage(destinationHash, path, formDataJson, timeoutSeconds)
