...
import network.libertychat.app.rns.host.ipc.BoundRnsBackend
import network.libertychat.app.rns.host.process.ProcessDetector
import network.libertychat.app.rns.host.process.ProcessType
 ...
 * - In the UI process (`network.libertychat.app[.debug]`) → return
 *   [BoundRnsBackend], an AIDL proxy that delegates every call to the
 *   `:reticulum`-process backend via the existing
 *   [network.libertychat.app.rns.host.ReticulumServiceConnection] +
 *   [network.libertychat.app.rns.ipc.RnsBackendClient] surface. The `Lazy` is
