...
class KotlinBLEBridgeTest {
    ...
    private fun addMockPeer(
        ...
        val peerConnectionClass =
            Class.forName(
                "network.libertychat.app.rns.host.ble.bridge.KotlinBLEBridge\$PeerConnection",
            )
        val deduplicationStateClass =
            Class.forName(
                "network.libertychat.app.rns.host.ble.bridge.KotlinBLEBridge\$DeduplicationState",
            )
    ...
    fun `pending connection notification uses current peer state after deduplication`() =
        runTest {
            ...
            val pendingConnectionClass =
                Class.forName(
                    "network.libertychat.app.rns.host.ble.bridge.KotlinBLEBridge\$PendingConnection",
                )
    ...
    fun `addConnectionChangeListener registers listener successfully`() =
        runTest {
            ...
            val bridge = createBridgeWithMockScanner()
            val listener =
                object : network.libertychat.app.rns.api.BleConnectionsListener {
            ...
            @Suppress("UNCHECKED_CAST")
            val listeners = listenersField.get(bridge) as MutableList<network.libertychat.app.rns.api.BleConnectionsListener>
