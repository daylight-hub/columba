...
import network.libertychat.app.rns.api.model.FileAttachment
...
class ParcelRoundTripTest {
    ...
    private fun makeLink(): Link {
        val identity = network.libertychat.app.rns.api.model.Identity(
            hash = ByteArray(16) { it.toByte() },
            publicKey = ByteArray(32) { it.toByte() },
            privateKey = null,
        )
        val destination = network.libertychat.app.rns.api.model.Destination(
            hash = ByteArray(16) { it.toByte() },
            hexHash = "0123456789abcdef",
            identity = identity,
            direction = network.libertychat.app.rns.api.model.Direction.OUT,
            type = network.libertychat.app.rns.api.model.DestinationType.SINGLE,
            appName = "test.app",
